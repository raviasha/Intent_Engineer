...
invariants must all validate before replacement. The Ubuntu job then invokes the trusted image adapter:
...
Snapshot and baseline bindings include file identities and change timestamps as well as bytes, so results
must be regenerated after code changes, even if the original bytes are restored, or in another
checkout. Dirty local test runs can report their process outcome but produce no committed-tree
...
CI uses a rootful Docker daemon and the installed adapter as trusted infrastructure. The adapter
...
The host and image entrypoints use isolated Python so project packages cannot shadow the adapter.
...
Every reviewed command, result finalization and final `CheckService` CI consumption runs in one
container pinned by its exact image ID. The process is non-root, network-disabled, capability-free,
...
The guarantee certifies the image's exact commit, not continued immutability of an external checkout.
...
for audit; host-side replay cannot establish CI eligibility. Trust is supplied over stdin,
not baked into the image or command arguments, and is cleared before reviewed commands. The
...
1. Review and activate the project baseline. Include a nonempty `test_commands` list in the
   ...
   with the installed test dependencies. A reviewed `#!/bin/sh` wrapper can run
