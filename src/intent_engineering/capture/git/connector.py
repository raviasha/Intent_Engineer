...
from intent_engineering.capture.base import RawSourceObject, SourceObject, normalize_raw_source
...
class GitConnector:
    ...
    async def fetch(self, object_id: str, version: str) -> RawSourceObject:
        """Fetch commit metadata and changed paths through the blocking Git boundary."""
        ...
        if len(fields) < 7 or fields[0] != sha:
            raise ValueError(f"unexpected Git commit metadata for: {object_id}")
        paths_output = await anyio.to_thread.run_sync(
            run_git,
            self.repo,
            ["diff-tree", "--root", "--no-commit-id", "--name-only", "-r", "-z", sha],
        )
        changed_paths = [path for path in paths_output.split("\x00") if path]
